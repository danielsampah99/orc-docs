...
export default function TableOfContents({ contentId }: { contentId: string }) {
	...
	return (
		<nav className="sticky top-16">
			<h2 className="text-sm/6 font-semibold text-gray-950">On this page</h2>
			<ul className="mt-3 flex flex-col gap-3 border-l border-gray-950/10 text-sm/6 text-gray-700">
				{headings.map((heading) => (
					<li
						key={heading.id}
						className={cn(
							"-ml-px border-l border-transparent pl-4",
							"hover:text-gray-950 hover:not-has-aria-[current=location]:border-gray-400",
							"has-aria-[current=location]:border-gray-95",
						)}
					>
						<a
							href={`#${heading.id}`}
							...
							className={cn(
								...
								"block aria-[current=location]:font-medium aria-[current=location]:text-gray-950",
							)}
						>
							{heading.text}
						</a>
